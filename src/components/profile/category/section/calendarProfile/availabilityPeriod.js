...
function AvailabilityPeriod(props) {
    ...
    const addNewSpecialDateInPrestationOrOption = (start, end) => {
        ...
        //     addAllNewSpecialDateTo(start, props_options, addAllUserOptions);
        ...
        if (selectedPrestation !== "all" && selectedPrestation !== null)
            ...
            addAllNewSpecialDateTo(start, prestations, addAllUserPrestation);
    };

    const applyModification = async (start, end) => {
        if (oneDate) await addNewSpecialDateInPrestationOrOption(start);
        else {
            if (calculateNumberDaysBetweenDates(new Date(end), new Date(start)) === 0)
                await addNewSpecialDateInPrestationOrOption(start);
            else await addNewSpecialDateInPrestationOrOption(start, end);
        }

        await updateOptionAndServiceWithSpecialDates()
    }
    ...
    const checkValidators = () => {
        ...
        if (oneDate && start === now)
            toast.error("Ne pas choisir la date d'aujourd'hui");
        ...
        else applyModification(start, end).then(r => null)
    };
    ...
    const updateOptionAndServiceWithSpecialDates = () => {
        ...
        if (selectedPrestation !== "all" && selectedPrestation !== null) {
            ...
            api_call_update.push(
                axios.put('api/artist_services/update/' + id, objectToFormData(selectedPrestationToUpdate)
                    , {headers: props.parentProps.headers}).then((resp) => {
    ...
    const changeDayOfAvailability = (day) => {
        ...
        axios.put('api/artist_conditions/update', tmp, {headers: props.parentProps.headers}
        ).then((err) => null)
    };

    const editEnable = (e) => {
        setDeActiveEdit(false);
        setEnable(e.target.value === "true")
    };
            ...
            <div className="form-row justify-content-center">
                <div className="text-center mb-3">
                    ...
                    <div className="countDown">
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Lundi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Mardi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Mercredi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Jeudi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Vendredi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Samedi</small>
                        </div>
                        <div className="btn btn-outline-primary bg-transparent cursor-pointer">
                            ...
                            <small
                                className={lightModeOn ? "count-type text-black" : "count-type text-white"}>Dimanche</small>
                        </div>
                    </div>
                </div>

                <div className="form-group text-center col-md-8 border-top pt-3">
                    <h4 className="text-red bolder pb-4">
                        Choisir une date ou une période d'indisponibilité / disponibilité
                    </h4>
                    <div className="row justify-content-center">
                        {oneDate || periodDate ? <div className="ml-2 mr-2">
                            ...
                            <DatePicker className={lightModeOn
                                ? "form-control text-black pl-5 pr-5"
                                : "form-control text-white pl-5 pr-5"}
                                        selected={startDate}
                                        minDate={new Date()}
                        ...
                        <div className="selecto">
                            <select name="slct"
                                ...
                                <option defaultValue={false} selected disabled>
                                    Choisir mon statut pour cette période
                                </option>
                ...
                <div className="form-group text-center col-md-8 border-bottom pb-3">
                    {applyStatus && <div>
                        <h4 className="text-red bolder pb-3">
                            Appliquer mon statut
                            <i className="icon ml-2 icon-info" data-tip data-for="statut"/>
                        </h4>
                        <div className="row justify-content-center">
                    ...
                    <button className="btn btn-outline-danger pl-5 pr-5 mt-4 ml-1 mr-1"
                            ...
                            onClick={checkValidators} disabled={deActiveEdit}>Appliquer les modifications
                    </button>}
